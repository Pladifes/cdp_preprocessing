...
def get_df_year_clean_2018(path_raw_data):
    """
    ...
    df_years = pd.read_excel(path_raw_data + "CDP_CC_emissions data_2018.xlsx", sheet_name="C0.2")
    ...
    df_clean = df_clean.merge(
        df_countries[["Account number", "C0.3_Select the countries/regions for which you will be supplying data."]],
        on="Account number",
    )
    ...
    df_clean.loc[df_clean["Account number"] == 22698, "accounting_year"] = [2015, 2016, 2017]
    df_clean.loc[df_clean["Account number"] == 8051, "accounting_year"] = [2017, 2016, 2015]
...
if __name__ == "__main__":
    ...
    df = get_df_year_clean_2018(path_raw_data)
    df.to_excel("temp_2018.xlsx")
