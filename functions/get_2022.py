...
from functions.utils import dict_to_relevance, get_accounting_year, keep_main_boundary


def get_df_year_clean_2022(path_raw_data):
    """
    ...
    df_years = pd.read_excel(path_raw_data + "CDP_CC_emissions data_2022.xlsx", sheet_name="C0.2")
    df_countries = pd.read_excel(
        path_raw_data + "CDP_CC_emissions data_" + str(year) + ".xlsx",
        sheet_name="C0.3",
    )
    df_boundaries = pd.read_excel(path_raw_data + "CDP_CC_emissions data_" + str(year) + ".xlsx", sheet_name="C0.5")
    ...
    # keep main boundaries
    col_boundary = "C0.5_Select the option that describes the reporting boundary for which climate-related impacts on your business are being reported. Note that this option should align with your chosen approach for consolidating your GHG inventory."
    df_boundaries[col_boundary] = df_boundaries[col_boundary].apply(keep_main_boundary)
    ...
    df_clean = df_clean.merge(df_base, on="Account number")
    ...
    df_clean = df_clean.merge(
        df_countries[["Account number", "C0.3_Select the countries/areas in which you operate."]],
        on="Account number",
    )
...
if __name__ == "__main__":
    ...
    df = get_df_year_clean_2022(path_raw_data)
    df.to_excel("temp_2022.xlsx")
